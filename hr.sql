...
--
-- Host: 127.0.0.1
-- Generation Time: May 27, 2020 at 02:31 PM
...
CREATE TABLE `locations` (
  ...
  `street_adress` varchar(25) DEFAULT NULL,
...
INSERT INTO `locations` (`location_id`, `street_adress`, `postal_code`, `city`, `state_province`, `country_id`) VALUES
...
(3200, 'Mariano Escobedo 9991', '11932', 'Mexico City', '\"Distrito Fe', 'MX');
...
SELECT first_name AS prénom, last_name_As nom FROM `employees` ;
...
SELECT CONCAT(first_name, " ", last_name) AS noms, salary, salary*0.15 AS CP FROM `employees`;

-- Écrire une requête pour obtenir l'ID de l'employé, les noms (prénom, nom), le salaire dans l'ordre croissant du salaire.
SELECT employee_id, CONCAT(first_name, " ", last_name) AS noms, salary FROM `employees` ORDER BY salary ASC;

-- Effectuez une recherche pour obtenir le total des salaires payables aux salariés.
SELECT SUM(salary) AS le_total_des_salaires_payables FROM `employees`;

-- Effectuez une recherche pour obtenir le salaire moyen et le nombre de salariés dans le tableau des salariés.
SELECT AVG(salary) AS le_salaire_moyen, COUNT(employee_id) AS le_nombre_de_salariés FROM `employees`;

-- Écrire une requête pour obtenir le nombre d'employés travaillant dans l'entreprise.
SELECT COUNT(employee_id) AS le_nombre_de_salariés FROM `employees`;
...
SELECT CONCAT(first_name, " ", last_name) AS noms, salary FROM `employees` WHERE salary NOT BETWEEN 10000 AND 15000;
...
SELECT CONCAT(first_name, " ", last_name) AS noms, department_id FROM `employees` WHERE department_id IN (30,100) ORDER BY department_id ASC;
...
SELECT CONCAT(first_name, " ", last_name) AS noms, department_id, salary FROM `employees` WHERE department_id IN (30,100) AND salary NOT BETWEEN 10000 AND 15000;
...
SELECT CONCAT(first_name, " ", last_name) AS noms, YEAR(hire_date) FROM `employees`;

-- Rédigez une requête pour trouver le nom (prénom, nom) de tous les employés qui travaillent dans le département informatique.
SELECT CONCAT(first_name, " ", last_name) AS noms, job_id FROM `employees` WHERE job_id LIKE "IT%";

-- Rédigez une requête pour trouver les adresses (ID_lieu, adresse_rue, ville, état_province, nom_pays) de tous les départements.
SELECT location_id, street_adress, city, state_province, country_name FROM `locations` NATURAL JOIN 'countries';
...
SELECT start_date, end_date, FLOOR(DATEDIFF(end_date, start_date)/365) FROM `job_history`;
